...
const INDEX_HTML = `
    ...
    <html lang="ru">
        <head>
            <meta charset="utf-8">
            <title>Последние новости с elpol.ru/news.asp</title>
        </head>
        <body>
            {{if .}}
                {{range .}}
                    {{.Time}}
                    <a href="http://elpol.ru/news.asp/{{.Ref}}">{{.Title}}</a>
                    <br/>
                {{end}}
            {{else}}
...
func main() {
	...
	log.Error("listener failed", "error", http.ListenAndServe("127.0.0.1:6064", nil))
}
